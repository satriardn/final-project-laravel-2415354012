<div class="modal fade" id="editDataModal" tabindex="-1" aria-labelledby="editDataModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" style="max-width: 650px; width: 100%; overflow: visible;">
        <div class="modal-content bg-white rounded-2xl p-6 shadow-xl border border-slate-100" style="overflow: visible;">
            <h2 class="text-xl font-bold text-slate-900 text-center mb-6">Edit Customer</h2>
            <form id="editCustomerForm" action="{{ session('edit_customer_id') ? route('customers.update', session('edit_customer_id')) : '' }}" method="POST">
                @csrf
                @method('PATCH')
                <div class="mb-4">
                    <label class="block font-semibold text-slate-700 text-sm mb-1.5">Customer ID</label>
                    <input type="hidden" id="edit_customer_db_id" name="db_id" value="{{ session('edit_customer_id', '') }}">
                    <input type="text" id="edit_customer_id" name="customer_id" value="{{ old('customer_id') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-800 placeholder-slate-400 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 focus:outline-none transition-all duration-200" placeholder="Enter your ID">
                    @error('customer_id') <div class="field-error" style="color: #ef4444; font-size: 13px; margin-top: 4px;">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="block font-semibold text-slate-700 text-sm mb-1.5">Customer Name</label>
                    <input type="text" id="edit_customer_name" name="name" value="{{ old('name') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-800 placeholder-slate-400 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 focus:outline-none transition-all duration-200" placeholder="Enter your name">
                    @error('name') <div class="field-error" style="color: #ef4444; font-size: 13px; margin-top: 4px;">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="block font-semibold text-slate-700 text-sm mb-1.5">Email</label>
                    <input type="email" id="edit_customer_email" name="email" value="{{ old('email') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-800 placeholder-slate-400 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 focus:outline-none transition-all duration-200" placeholder="Enter your email">
                    @error('email') <div class="field-error" style="color: #ef4444; font-size: 13px; margin-top: 4px;">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="block font-semibold text-slate-700 text-sm mb-1.5">Phone</label>
                    <input type="text" id="edit_customer_phone" name="phone" value="{{ old('phone') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-800 placeholder-slate-400 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 focus:outline-none transition-all duration-200" placeholder="Enter your phone">
                    @error('phone') <div class="field-error" style="color: #ef4444; font-size: 13px; margin-top: 4px;">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="block font-semibold text-slate-700 text-sm mb-1.5">Address</label>
                    <input type="text" id="edit_customer_address" name="address" value="{{ old('address') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-800 placeholder-slate-400 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 focus:outline-none transition-all duration-200" placeholder="Enter your address">
