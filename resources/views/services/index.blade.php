    ...
    <table class="w-full text-left border-collapse" style="overflow: visible;">
        <thead>
            <tr class="bg-slate-50/70 border-b border-slate-200/60">
                <th class="px-6 py-4 font-semibold text-slate-500 uppercase tracking-wider text-xs">Service Name</th>
                <th class="px-6 py-4 font-semibold text-slate-500 uppercase tracking-wider text-xs">Price</th>
        ...
        <tbody>
            @foreach ($services as $service)
                <tr class="border-b border-slate-100 hover:bg-slate-50/40 transition-colors">
                    <td class="px-6 py-4 text-slate-900 font-semibold">{{ $service['name'] }}</td>
                    <td class="px-6 py-4 text-slate-900 font-medium">Rp{{ number_format($service['price'], 0, ',', '.') }}</td>
                    <td class="px-6 py-4">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold {{ $service['status'] ? 'bg-emerald-50 text-emerald-700 border border-emerald-200/40' : 'bg-rose-50 text-rose-700 border border-rose-200/40' }}">
                            <span class="w-1.5 h-1.5 rounded-full {{ $service['status'] ? 'bg-emerald-500' : 'bg-rose-500' }}"></span>
